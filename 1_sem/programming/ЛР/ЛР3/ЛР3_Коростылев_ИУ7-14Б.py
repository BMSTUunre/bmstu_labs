...
po_ab = (ab + ao + bo) / 2  # полупериметр треугольника OAB
so_ab = sqrt(po_ab * (po_ab - ab) * (po_ab - ao) * (po_ab - bo))  # площадь треугольника OAB

po_bc = (bc + bo + co) / 2  # полупериметр треугольника OBC
so_bc = sqrt(po_bc * (po_bc - bc) * (po_bc - bo) * (po_bc - co))  # площадь треугольника OBC

po_ac = (ac + ao + co) / 2  # полупериметр треугольника OAC
...
# Будем находить наименьшую высоту трех, вышеупомянутых треугольниках.
h_to_ab = so_ab * 2 / ab
h_to_bc = so_bc * 2 / bc
h_to_ac = so_ac * 2 / ac
if h_to_ab <= h_to_bc:
