...
def main():
    ...
    n1, n2 = input_int('Введите N1 участков разбиения\n> ', False), input_int('Введите N2 участков разбиения\n> ', False)
    eps = input_float('Введите значение точности вычислений\n> ', True, True)


    rect1, rect2 = middle_rectangles(n1, start, stop, eps), middle_rectangles(n2, start, stop, eps)
    par1, par2 = parabola(n1, start, stop, eps), parabola(n2, start, stop, eps)
    ...
    if par1_err[0] == best_calc or par2_err[0] == best_calc:
        ...
        calc_n = iteration_calc_n(0, start, stop, eps)
        new_value = middle_rectangles(calc_n, start, stop, eps)

    else:
        print("\nМетод серединных прямоугольников оказался наиболее точным.")
        calc_n = iteration_calc_n(1, start, stop, eps)
        new_value = parabola(calc_n, start, stop, eps)
